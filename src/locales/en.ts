...
export const en: LocaleDict = {
  ...
  runtime: {
    ...
    delegationHintWhenUseful:
      "\n\n**Delegating to a sub-agent** (`consult_secondary_agent`, `allow_tools: true`): hand off work that is LARGE, INDEPENDENT, or benefits from an isolated context — a whole feature, a multi-file refactor, or a self-contained research thread. The sub-agent runs its own tool loop and AUTO-SAVES any files it creates (don't re-save them — verify with `list_directory`/`read_file`).\n\n**Do it yourself — do NOT delegate** — a single analysis, a file read, a small edit, a quick lookup, or a question you can answer directly. Delegating those just adds latency.\n\n**When you do delegate:** gather the needed context first (`list_directory`/`read_file`) and pass it explicitly — the sub-agent cannot see this conversation.\n",
