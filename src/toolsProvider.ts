...
let sharedAgentLoop: ((role: string, task: string, context: string, loopLimit: number, forceTools: boolean, cwd: string, taskAllowTools: boolean) => Promise<any>) | null = null;
...
export const toolsProvider: ToolsProvider = async (ctl) => {
  ...
  const skillCandidates = getSkillsDirCandidates(dirname(__dirname), currentWorkingDirectory);
  const loadedSkills = await loadSkillsCached(skillCandidates);
  if (loadedSkills.length > 0) {
    const skillNames = loadedSkills.map(s => s.name) as [string, ...string[]];
    const enumDesc = loadedSkills.map(s => `${s.name} (${s.description})`).join("; ");
    const useWorkflowTool = tool({
      name: "use_workflow",
      description:
  ...
  if (allowGit) {
    ...
    const gitCommitTool = tool({
      name: "git_commit",
      description: "Commit STAGED changes to the git repository. Only commits what has been staged with git_add — unstaged files are NOT included. Always call git_add first, then git_commit.",
      parameters: {
        message: z.string(),
      },
  ...
  const saveFileTool = tool({
    name: "save_file",
    description: text`
      Create a new file or COMPLETELY OVERWRITE an existing file in the current working directory.
      WARNING: This replaces the entire file content — all previous content is lost.
      For partial edits, use replace_text_in_file, insert_at_line, or delete_lines_in_file instead.
      Use save_file only for new files or when you intend to replace the full content.
    `,
    parameters: {
    ...
    implementation: async ({ query, queries, providers }) => {
      ...
      const searchFunctions: Record<SearchProvider, (q: string) => Promise<SearchResult[]>> = {
        ...
        "duckduckgo-html": async (q: string) => {
          ...
          const html = await page.goto(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(q)}`, { waitUntil: "networkidle2", timeout: 15000 })
            .then(() => page.content());
          await page.close();
  ...
  const getCurrentDatetimeTool = tool({
    ...
    description: "Get the current date and time. Use this tool whenever the user asks what time it is, what today's date is, what day of the week it is, or any question about the current date/time. This is the ONLY correct tool for date and time — do not use execute_command, run_python, or run_javascript for this.",
    parameters: {
      timezone: z.string().optional().describe("IANA timezone name (e.g. 'America/New_York'). Defaults to the system local timezone."),
      format: z.enum(["iso", "locale", "both"]).optional().describe("Output format: 'iso' for ISO 8601, 'locale' for human-readable, 'both' for both. Defaults to 'both'."),
    },
    implementation: async ({ timezone, format = "both" }) => {
      const now = new Date();
      ...
      if (format === "iso") return { iso: isoString, timezone: tz };
      if (format === "locale") return { datetime: localeString, timezone: tz };
      return { iso: isoString, datetime: localeString, timezone: tz };
    },
  });
  ...
  const runTestCommandTool = tool({
    ...
    description: "Run a test suite and return full output with exit code. Use this — NOT execute_command — for ALL test execution (npm test, pytest, go test ./..., jest, mocha, etc.). Always call this to verify work before declaring a task complete.",
    parameters: {
  ...
  const consultSecondaryAgentTool = tool({
    ...
    description: "Delegate a task to a secondary agent with its own tool loop. Use for: implementing a plan task with TDD, code review (spec compliance or quality), research, or any work that benefits from isolated context. IMPORTANT: If the task produces files, the sub-agent SAVES THEM AUTOMATICALLY — do NOT save them yourself afterward. Verify via list_directory or read_file. If output says '[System: Code Block Hidden for Brevity...]' that is a SUCCESS — the file is on disk.",
    parameters: {
      task: z.string().describe("Clear, self-contained task description. Include everything the sub-agent needs — it has NO access to your conversation history."),
      agent_role: z.string().optional().describe("Sub-agent persona. Use 'general' (default), 'coder' (implementation), or 'reviewer' (code review and bug fixing)."),
      context: z.string().optional().describe("Paste relevant file contents, code snippets, error messages, or background info here. The sub-agent cannot read your conversation — provide everything it needs explicitly."),
      allow_tools: z.boolean().optional().describe("If true, the sub-agent can use file system, web search, and code execution tools (per plugin settings). Set true for implementation and research tasks. Default: false."),
    },
    implementation: createSafeToolImplementation(
...
Always assume relative paths are from this directory.`;
          ...
          // Append Tools
          let toolsReminder = "";
          const toolsEnabled = taskAllowTools || forceTools;
          if (toolsEnabled) {
            const allowedTools = [];
            if (allowFileSystem) allowedTools.push("read_file", "list_directory", "save_file", "replace_text_in_file", "delete_files_by_pattern", "fuzzy_find_local_files", "search_in_file");
          ...
          // Use SDK directly when targeting localhost (no local server toggle needed).
          // Fall back to REST API for external endpoints.
          const useSDK = Boolean(client) && (useMainModel || endpoint.startsWith("http://localhost") || endpoint.startsWith("http://127.0.0.1"));
          ...
          while (loops < loopLimit) {
            try {
              ...
              if (useSDK) {
                // SDK path: bypasses the LM Studio Local Server requirement.
                try {
                  const sdkModelId = modelId && modelId !== "local-model" ? modelId : undefined;
                  const sdkLLM = client as LMStudioClient;
                  const modelHandle = sdkModelId
                    ? await sdkLLM.llm.model(sdkModelId)
                    : await sdkLLM.llm.model();
                  const sdkResult = await modelHandle.respond(msgList as any, { temperature: 0.7 });
                  message = { role: "assistant", content: sdkResult.content };
                  if (subAgentDebugLogging) {
                    console.log(`[Sub-Agent] SDK respond ok, length=${sdkResult.content.length}`);
                  }
                } catch (sdkErr: any) {
                  if (subAgentDebugLogging) {
                    console.log(`[Sub-Agent] SDK failed (${sdkErr?.message}), falling back to REST`);
                  }
                  // Fall back to REST API
                  const response2 = await fetch(`${endpoint}/chat/completions`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ model: modelId, messages: msgList, temperature: 0.7, stream: false })
                  });
                  if (!response2.ok) {
                    const errorBody2 = await response2.text().catch(() => "");
                    const compactErrorBody2 = errorBody2.replace(/\s+/g, " ").trim().substring(0, 600);
                    const details2 = compactErrorBody2 ? ` - ${compactErrorBody2}` : "";
                    return { error: `API Error: ${response2.status}${details2}`, filesModified };
                  }
                  const data2 = await response2.json();
                  message = data2?.choices?.[0]?.message;
                }
              } else {
                // External endpoint — use REST API as before.
              ...
              if (toolCall && toolCall.tool) {
                ...
                try {
                  ...
                  if (allowFileSystem) {
                    ...
                    } else if (toolCall.tool === "rag_local_files" && toolCall.args?.query) {
                      if (!client) {
                        toolResult = "Error: LM Studio client unavailable for RAG.";
                      } else {
                        try {
                          const ragQuery = toolCall.args.query;
                          const ragPath = toolCall.args.path || ".";
                          const ragTargetDir = validatePath(currentWorkingDirectory, ragPath);
                          const ragEntries = await readdir(ragTargetDir, { recursive: true, withFileTypes: true });
                          const ragFiles = ragEntries.filter(e => e.isFile() && !e.name.match(/\.(png|jpg|jpeg|gif|ico|exe|dll|bin)$/i));
                          const ragFiltered = toolCall.args.file_pattern
                            ? ragFiles.filter(e => e.name.includes(toolCall.args.file_pattern) || join(e.parentPath || e.path, e.name).includes(toolCall.args.file_pattern))
                            : ragFiles;
                          const ragEmbModel = await client.embedding.model(embeddingModelName);
                          const [ragQueryEmb] = await ragEmbModel.embed([ragQuery]);
                          let ragChunks: { chunk: string; score: number; file: string }[] = [];
                          for (const file of ragFiltered.slice(0, 50)) {
                            try {
                              const fp = join(file.parentPath || (file as any).path, file.name);
                              const fc = await readFile(fp, "utf-8");
                              const chunks = fc.split(/\n\s*\n/).filter(c => c.trim().length > 20);
                              const chunkEmbs = await ragEmbModel.embed(chunks);
                              chunks.forEach((chunk, i) => {
                                const score = cosineSimilarity(ragQueryEmb.embedding, chunkEmbs[i].embedding);
                                if (score > 0.4) ragChunks.push({ chunk, score, file: file.name });
                              });
                            } catch { /* skip unreadable files */ }
                          }
                          ragChunks.sort((a, b) => b.score - a.score);
                          toolResult = JSON.stringify({ query: ragQuery, results: ragChunks.slice(0, 10).map(c => ({ file: c.file, score: c.score.toFixed(3), content: c.chunk })) });
                        } catch (e) {
                          toolResult = `rag_local_files failed: ${e instanceof Error ? e.message : String(e)}`;
                        }
                      }
                    } else if (toolCall.tool === "search_in_file" && toolCall.args?.file_name && toolCall.args?.pattern) {
                      const fpath = validatePath(currentWorkingDirectory, toolCall.args.file_name);
                      const fileContent = await readFile(fpath, "utf-8");
                      const lines = fileContent.split("\n");
                      const caseSensitive = toolCall.args.case_sensitive === true;
                      const useRegex = toolCall.args.use_regex === true;
                      const searchMatches: Array<{ line_number: number; content: string }> = [];
                      for (let i = 0; i < lines.length; i++) {
                        const line = caseSensitive ? lines[i] : lines[i].toLowerCase();
                        const pat = caseSensitive ? toolCall.args.pattern : toolCall.args.pattern.toLowerCase();
                        const isMatch = useRegex ? new RegExp(pat).test(line) : line.includes(pat);
                        if (isMatch) searchMatches.push({ line_number: i + 1, content: lines[i] });
                      }
                      toolResult = JSON.stringify({ matches: searchMatches, total: searchMatches.length });
                  ...
                  if (allowWeb && !toolResult) {
                    if (toolCall.tool === "wikipedia_search") {
                      const lang = toolCall.args?.lang ?? "en";
                      ...
                      try {
                        ...
                        if (!searchData.query?.search?.length) {
                          ...
                          toolResult = JSON.stringify({ results: wikiResults });
                        }
                      } catch (e) {
                        toolResult = `Wikipedia search failed: ${e instanceof Error ? e.message : String(e)}`;
                      }
                      ...
                      if (!client) {
                        toolResult = "Error: LM Studio client not available for RAG.";
                      } else {
                        try {
                          const ragRes = await fetch(toolCall.args.url);
                          if (!ragRes.ok) throw new Error(`HTTP ${ragRes.status}`);
                          let rawText = await ragRes.text();
                          const { compile } = await import("html-to-text");
                          rawText = compile({ wordwrap: false, selectors: [{ selector: "a", options: { ignoreHref: true } }, { selector: "img", format: "skip" }] })(rawText);
                          const ragResults = await performRagOnText(rawText, toolCall.args.query, client, embeddingModelName);
                          toolResult = JSON.stringify({ url: toolCall.args.url, query: toolCall.args.query, relevant_chunks: ragResults });
                        } catch (e) {
                          toolResult = `rag_web_content failed: ${e instanceof Error ? e.message : String(e)}`;
                        }
                      }
                  ...
                  if (allowCode && !toolResult) {
                    if (toolCall.tool === "run_python") {
                      ...
                      const res = await originalRunJavascriptImplementation({ javascript: toolCall.args.javascript });
                      toolResult = res.stderr ? `Error: ${res.stderr}` : res.stdout;
                    }
                  }
        ...
        sharedAgentLoop = runAgentLoop;
        const primaryResult = await runAgentLoop(agent_role, task, context, 8, false, currentWorkingDirectory, allow_tools);
        ...
        if (debugMode && primaryResult.filesModified.length > 0) {
          ...
          const debugResult = await runAgentLoop("reviewer", debugTask, debugContext, 5, true, currentWorkingDirectory, true);
  ...
  const dispatchParallelAgentsTool = tool({
    name: "dispatch_parallel_agents",
    description: "Dispatch 2-8 INDEPENDENT tasks to sub-agents running in parallel. Use this when you have multiple problems that have no shared state and can be solved concurrently — different files, different subsystems, different bugs with unrelated root causes. All tasks run simultaneously and results are collected. Do NOT use when tasks share state or must happen sequentially.",
    parameters: {
      tasks: z.array(z.object({
        task: z.string().describe("Self-contained task description. Include all context — the agent has no conversation history."),
        agent_role: z.string().optional().describe("Agent persona: 'general' (default), 'coder', or 'reviewer'."),
        context: z.string().optional().describe("Relevant file contents, error messages, or background. The agent cannot read your conversation."),
        allow_tools: z.boolean().optional().describe("Allow file system and web tools. Set true for implementation/research tasks."),
      })).min(2).max(8).describe("Array of independent tasks to run in parallel. Must have at least 2."),
    },
    implementation: createSafeToolImplementation(
      async ({ tasks }) => {
        if (!sharedAgentLoop) {
          return { error: "Parallel dispatch requires consult_secondary_agent to have been called at least once. Use consult_secondary_agent first to initialize the agent system, or enable Secondary Agent in plugin settings." };
        }
        const settled = await Promise.allSettled(
          tasks.map(t => sharedAgentLoop!(
            t.agent_role ?? "general",
            t.task,
            t.context ?? "",
            8,
            false,
        ...
        const results = settled.map((r, i) => {
          if (r.status === "fulfilled") {
            return {
              ...
              files_modified: r.value.filesModified || [],
              handoff_message: r.value.handoff_message,
            };
          } else {
            return {
              task_index: i,
              status: "failed",
              error: (r.reason as any)?.message || String(r.reason) || "unknown error",
            };
          }
        });
        const completed = results.filter(r => r.status === "completed").length;
        const failed = results.filter(r => r.status === "failed").length;
        const allFiles = results.flatMap(r => (r as any).files_modified || []);
        return {
          summary: `${completed}/${tasks.length} tasks completed${failed > 0 ? `, ${failed} failed` : ""}.${allFiles.length > 0 ? ` Files modified: ${allFiles.join(", ")}` : ""}`,
          results,
        };
      },
      enableSecondary,
  ...
  tools.push(dispatchParallelAgentsTool);
  ...
  const insertAtLineTool = tool({
    name: "insert_at_line",
    description: text`
      Insert content at a specific line number in a file (existing lines are pushed down, nothing is deleted).
      PREFERRED over replace_text_in_file for inserting new code blocks because it cannot fail on whitespace mismatches.
      Use search_in_file first to find the target line number, then insert here.
  ...
  const readFileRangeTool = tool({
    ...
    description: text`
      Read a specific range of lines from a file. Returns content with line numbers.
      PREFER this over read_file for any file larger than ~50 lines — it saves context window.
      Workflow: call search_in_file to find the line number you need, then read_file_range with ±10 lines of context.
      Line numbers are 1-indexed (line 1 is the first line).
    `,
    parameters: {
      file_name: z.string(),
  ...
  const searchInFileTool = tool({
    name: "search_in_file",
    description: text`
      Search for a pattern within a single file and return matching lines with their line numbers.
      PRIMARY SEARCH TOOL — always use this before read_file_range to locate the exact lines you need.
      Use this to find function definitions, class names, variable declarations, imports, or any specific text.
      After getting line numbers here, use read_file_range(start-10, end+10) to read only the relevant context.
    `,
    parameters: {
      file_name: z.string(),
