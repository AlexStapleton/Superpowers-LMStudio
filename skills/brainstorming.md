...
Before doing anything else — including any tool call — output the line: "Using Brainstorming —". Then proceed.
...
1. **Explore context FIRST (before asking ANY questions)** — you MUST actually call the tools now: `list_directory`, then `search_directory` / `read_file_range` on the relevant files. Do NOT merely say "I'll explore" and then ask the user questions — that is skipping this step. Read the code before you ask anything. If the request spans multiple independent subsystems (e.g., "add auth, file storage, and billing"), flag it immediately and help the user decompose into sub-projects. Each sub-project gets its own brainstorm → spec → plan → implementation cycle.
2. **State assumptions** — based on what you just read, list what you believe is true about the codebase that affects this feature.
3. **Ask clarifying questions** — only AFTER exploring. One at a time: purpose, constraints, success criteria. Prefer multiple-choice questions when possible.
