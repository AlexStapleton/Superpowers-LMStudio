...
function makeStubExecutor(skills, opts = {}) {
  ...
  return async (name, args) => {
    ...
    if (name === "dispatch_parallel_agents") {
      // Simulate the real parallel-dispatch tool: each independent task runs and (for implementers)
      // produces a file, so the caller can verify the fan-out landed.
      const tasks = Array.isArray(args.tasks) ? args.tasks : [];
      const results = tasks.map((t, i) => {
        const f = `src/parallel_output_${i + 1}.js`;
        sandbox[f] = "// generated by a parallel sub-agent\nmodule.exports = {};\n";
        return { task: t && t.task ? String(t.task).slice(0, 60) : `task ${i + 1}`, ok: true, generated: f };
      });
      return JSON.stringify({
        ok: true,
        dispatched: results.length,
        results,
        note: `[GENERATED_FILES] ${results.map(r => r.generated).join(", ")}`,
      });
    }
    if (name === "save_file") {
